...
@section('content')
    ...
    <div class="pt-5 bg-black">
        ...
        <div class="slider d-flex justify-content-between pb-4">
            @foreach ($category_detail_list as $category_detail_list)
                <a href="{{ route('category_detail', $category_detail_list->slug) }}">
                    <div class="">
                        <div class="position-relative bg-color slide-item">
                            <img class="slide-img"
                                src="{{ asset('storage/' . $category_detail_list->image) }}" alt=""
                                width="329px" height="329px">
                            <h5 class="text-uppercase text-rotate position-absolute text-slide text-black">
                                {{ $category_detail_list->name }}</h5>
                        </div>
                    </div>
                </a>
            @endforeach
